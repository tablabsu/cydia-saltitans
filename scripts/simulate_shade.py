...
ax.set_xlabel(f'Rate (N = {args.get("num_beans")})')
ax.set_ylabel("Frequency")
#ax.set_title("Rate of finding shade (combined)")
plt.hist(diffuse_times, label="Diffuse", bins=np.logspace(np.log10(0.00001), np.log10(10)), density=True, color=(1, 0, 0), histtype='step', linewidth=3)
plt.hist(ballistic_times, label="Ballistic", bins=np.logspace(np.log10(0.00001), np.log10(10)), density=True, color=(0, 0, 1), histtype='step')
plt.legend()
plt.show()
if input("Save figure? ").lower() in ('y', 'yes'):
    logging.info("Saving figure...")
    fig.savefig("figure-shade-combined")